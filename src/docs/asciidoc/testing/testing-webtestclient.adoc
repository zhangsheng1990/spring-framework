...
----
	client.get().uri("/persons")
			.exchange()
			.expectStatus().isOk()
			.expectBodyList(pojo.Person.class).hasSize(3).contains(person);
----
====
...
====
----
	EntityExchangeResult<pojo.Person> result = client.get().uri("/persons/1")
			.exchange()
			.expectStatus().isOk()
			.expectBody(pojo.Person.class)
			.returnResult();
