...
[source,kotlin,indent=0]
----
	class pojo.Person(val name: String, val age: Int)
----
====
...
As the following example shows, this allows for easy changes to individual properties, even if `pojo.Person` properties are read-only:

====
[source,kotlin,indent=0]
----
	data class pojo.Person(val name: String, val age: Int)

	val jack = pojo.Person(name = "Jack", age = 1)
