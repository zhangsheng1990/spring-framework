...
The next example provides validation behavior for the `pojo.Person` class by implementing the
...
example implements `Validator` for `pojo.Person` instances:
...
----
	public class PersonValidator implements Validator {
		...
		/**
		 * This Validator validates *only* pojo.Person instances
		 */
		public boolean supports(Class clazz) {
			return pojo.Person.class.equals(clazz);
		}

		public void validate(Object obj, Errors e) {
			ValidationUtils.rejectIfEmpty(e, "name", "name.empty");
			pojo.Person p = (pojo.Person) obj;
			if (p.getAge() < 0) {
