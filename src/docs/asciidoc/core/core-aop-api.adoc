...
----
	...
	<bean id="person"
		...
		<property name="proxyInterfaces" value="com.mycompany.pojo.Person"/>
...
The `person` bean definition shown earlier can be used in place of a `pojo.Person` implementation, as
follows:
...
[subs="verbatim,quotes"]
----
	pojo.Person person = (pojo.Person) factory.getBean("person");
----
====
...
The `PersonUser` class in this example exposes a property of type `pojo.Person`. As far as
...
Using an anonymous inner bean has the advantage that there is only one object of type `pojo.Person`. This is useful if we want
...
Imagine that in our earlier example, there was no `pojo.Person` interface. We needed to advise
a class called `pojo.Person` that did not implement any business interface. In this case, you
